...
char AllAppt(P_CALENDAR date, int maxDays) { // displays all appointments
...
char RangeAppt(P_CALENDAR date, int maxDays) { // displays a range of appointments
	...
	} while ((scanf("%d", &firstDay) != 1) || (firstDay < 0) || (firstDay > maxDays));
	fseek(stdin, 0, SEEK_END);
	...
	} while ((scanf("%d", &lastDay) != 1) || (lastDay < 0) || (lastDay > maxDays));
	fseek(stdin, 0, SEEK_END);
	printf("\n");
...
char SingleAppt(P_CALENDAR date, int maxDays) { // displays a chosen appointment
	...
	} while ((scanf("%d", &dayChoice) != 1) || (dayChoice < 0) || (dayChoice > maxDays));
	fseek(stdin, 0, SEEK_END);
	printf("\n");
